...
Windows 上需要以 LibreOffice 檢查 PDF 版面時，請使用 `.\scripts\convert_docx_to_pdf.ps1 -InputPath <docx> -OutputDirectory <資料夾>`。腳本會將暫存 profile 轉為正確的 `file:///C:/...` URI，避免錯誤的 Windows Path 被誤判為 `bootstrap.ini` 損壞。

...
> 最後更新：2026-07-17。所有後續實作開始前都必須先閱讀本文件與 `AGENTS.md`；完成後須更新本文件、驗證、提交並推送。
...
- 最近完整驗證：2026-07-17 已通過 `manage.py check`、`manage.py makemigrations --check --dry-run` 與 65 項 `manage.py test`；另以 10 筆評量、12 筆得獎及 15 筆輔導紀錄驗證 DOCX 動態列、重複表頭與不可拆列設定。
